...
list.of.packages <- c("haven", "dplyr", "tidyverse","ltm","jmv", "apaTables", "arsenal", "ggstatsplot","QuantPsyc","devtools")
# for APA citation style t-test tables:
...
df$ManiCheck_mean <- df %>% dplyr::select(starts_with("ManiCheck") & !contains("DO")) %>% rowMeans()
# get rid of order marking variables like "ManiCheck_esteem_DO_1"
...
df$MASC_set1_mean <- df %>% dplyr::select(starts_with("MASC_set1")) %>% rowMeans()
df$MASC_set2_mean <- df %>% dplyr::select(starts_with("MASC_set2")) %>% rowMeans()
df$MASC_set3_mean <- df %>% dplyr::select(starts_with("MASC_set3")) %>% rowMeans()
```
...
BIDR_self_deceptive_odd <- df %>% 
  dplyr::select(starts_with("BIDR_Self_deceptive") & !contains("DO"))  %>%
  dplyr::select(ends_with("0") | ends_with("2") | ends_with("4") | ends_with("6") | ends_with("8")) 
# de-select order marking variables like"BIDR_Self_deceptive_DO_20"
...
BIDR_self_deceptive_even <- df %>% 
  dplyr::select(starts_with("BIDR_Self_deceptive") & !contains("DO")) %>%
  dplyr::select(ends_with("1") | ends_with("3") | ends_with("5") | ends_with("7") | ends_with("9")) 
...
BIDR_impre_manage_even <- df %>% 
  dplyr::select(starts_with("BIDR_Impre_manage") & !contains("DO")) %>%
  dplyr::select(ends_with("0") | ends_with("2") | ends_with("4") | ends_with("6") | ends_with("8")) 
...
ethi_dis_write <- df %>% filter(FL_9_DO_RecallManipulation_EthicalDissonancebyWriting_UnethicalB == 1) %>%
  dplyr::select(RecallComprDissoWri:RecallCheck2DissoWri, # recall manipulation and comprehension check
         "study_order",
         starts_with("ManiCheck"), # includes precalculated average and original items for calculating Cronbach's alpha
         starts_with("Exp1"),
         starts_with("Exp2"),
         starts_with("MASC"), 
         "BIDR_self_deceptive","BIDR_impre_manage", 
         age:CountryName, Duration__in_seconds_) %>% #demographic data and condition marker
  dplyr::select(!contains("DO")) %>% # screen out order marking variables
  mutate(condition = "Dissonance_write")

# check comprehension of recall task
...
# consistently rename recall variables before  combining dataframe
colnames(ethi_dis_write)[1:4] <- c("Recall_comprehension", "Recall_writing", "Recall_check1", "Recall_check2")
colnames(ethi_dis_nowrite)[1:4] <- c("Recall_comprehension", "Recall_writing", "Recall_check1", "Recall_check2")
colnames(con_worthy)[1:4] <- c("Recall_comprehension", "Recall_writing", "Recall_check1", "Recall_check2")
colnames(con_neutral)[1:4] <- c("Recall_comprehension", "Recall_writing", "Recall_check1", "Recall_check2")
colnames(con_nega)[1:4] <- c("Recall_comprehension", "Recall_writing", "Recall_check1", "Recall_check2")

cleaned_df <- ethi_dis_write %>% 
  rbind(ethi_dis_nowrite) %>%
  rbind(con_worthy) %>%
  rbind(con_neutral) %>%
  rbind(con_nega) 
# 930 -> 765
...
# output cleaned data
write.csv(cleaned_df, "cleaned_data_0730_screened.csv",fileEncoding = "UTF-8")
```
...
# Manipulation Check
## overall
jmv::descriptives(data = cleaned_df, vars = vars(ManiCheck_mean)) 
## by condition
jmv::descriptives(
    formula = ManiCheck_mean ~ condition,
    data = cleaned_df,
    missing = FALSE,
    median = FALSE,
    variance = TRUE,
...
jmv::descriptives(
    data = cleaned_df,
    vars = vars(MASC_set1_mean, MASC_set2_mean, MASC_set3_mean, BIDR_self_deceptive, BIDR_impre_manage))
## condition
jmv::descriptives(
    formula = MASC_set1_mean + MASC_set2_mean + MASC_set3_mean + BIDR_self_deceptive + BIDR_impre_manage ~ condition,
    data = cleaned_df,
    missing = FALSE, median = FALSE)
...
jmv::descriptives(
    ...
    freq = TRUE)
# gender: 1 - male, 2- female, 3 - other, 4 - rather not disclose
...
#Cronbach's alpha of 3 raw items
cleaned_df %>% dplyr::select(starts_with("ManiCheck") & !contains("mean")) %>% cronbach.alpha(standardized=TRUE,CI=TRUE)
...
ANOVA_mani_check <- lm(ManiCheck_mean ~ condition, data = cleaned_df)
...
# manually calculate CI for effect size
# sample code from https://dstanley4.github.io/apaTables/reference/get.ci.partial.eta.squared.html
get.ci.partial.eta.squared(2.45, 4, 760, conf.level = 0.9)
```
...
# manually calculate CI for effect size
# sample code from https://dstanley4.github.io/apaTables/reference/get.ci.partial.eta.squared.html
get.ci.partial.eta.squared(4.29, 4, 760, conf.level = 0.9)
...
# manually calculate CI for effect size
# sample code from https://dstanley4.github.io/apaTables/reference/get.ci.partial.eta.squared.html
get.ci.partial.eta.squared(4.00, 4, 760, conf.level = 0.9)
```
...
Calculate t tests or ANOVA according to original reporting. 
T-test sample code from https://rempsyc.remi-theriault.com/articles/t-test.html
...
#Cronbach alpha of 8 raw items
cleaned_df %>% dplyr::select(starts_with("MASC_set1") & !contains("mean")) %>% cronbach.alpha(standardized=TRUE,CI=TRUE)

## MASC set 2
#Cronbach alpha of 6 raw items
cleaned_df %>% dplyr::select(starts_with("MASC_set2") & !contains("mean")) %>% cronbach.alpha(standardized=TRUE,CI=TRUE)

# t-test for set 1-3
Study3_main <- ethi_dis_write %>% 
  rbind(con_worthy)

nice_t_test(data = Study3_main,
            response = c("MASC_set1_mean","MASC_set2_mean","MASC_set3_mean"),
            group = "condition",
            warning = FALSE) -> MASC_t_tests
# reformat results
MASC_t_table <- nice_table(MASC_t_tests)
MASC_t_table
save_as_docx(MASC_t_table,path = "MASC_t_tests.docx")
```


```{R Study3 MASC1 output, eval=FALSE, include=FALSE}

# cancelled 

# ANOVA
jmv::ANOVA(
    formula = MASC_set1_mean ~ condition,
    data = cleaned_df,
    effectSize = "partEta",
    homo = TRUE,
    norm = TRUE,
...
ANOVA_study3_MASC1 <- lm(MASC_set1_mean ~ condition, data = cleaned_df)
...
```{R Study3 set2 output, eval=FALSE, include=FALSE}

# cancelled 

## MASC set 2
jmv::ANOVA(
    formula = MASC_set2_mean ~ condition,
    data = cleaned_df,
    effectSize = "partEta",
    homo = TRUE,
    norm = TRUE,
...
ANOVA_study3_MASC2 <- lm(MASC_set2_mean ~ condition, data = cleaned_df)
...
# using only conditions in original study

## MASC set 3
jmv::ANOVA(
    formula = MASC_set3_mean ~ condition,
    data = Study3_main,
    effectSize = "partEta",
    homo = TRUE,
    norm = TRUE,
...
ANOVA_study3_MASC3 <- lm(MASC_set3_mean ~ condition, data = Study3_main)
...
Calculate t tests according to original study.
```{r}
nice_t_test(data = Study3_main,
            response = c("BIDR_self_deceptive","BIDR_impre_manage"),
            group = "condition",
            warning = FALSE) -> BIDR_t_tests
# reformat results
BIDR_t_table <- nice_table(BIDR_t_tests)
BIDR_t_table
save_as_docx(BIDR_t_table,path = "BIDR_t_tests.docx")
```

```{r BIDR self ANOVA, eval=FALSE, include=FALSE}

"Notably, impression management scores (M = 7.34, SD = 4.56) are higher than self-deceptive enhancement scores (M = 3.57, SD = 3.12) across all conditions, which possibly indicate a deliberate effort of impression management of all participants in the online survey setting. "
# cancelled 
## BIDR - self deceptive
jmv::ANOVA(
...
```{R Study3 BIDR impression ANOVA, eval=FALSE, include=FALSE}
## cancelled 
...
ANOVA_exp3_MASC1 <- lm(MASC_set1_mean ~ condition, data = cleaned_df)
summary.lm(ANOVA_exp3_MASC1)

ANOVA_exp3_MASC2 <- lm(MASC_set2_mean ~ condition, data = cleaned_df)
summary.lm(ANOVA_exp3_MASC2)

ANOVA_exp3_MASC3 <- lm(MASC_set3_mean ~ condition, data = cleaned_df)
