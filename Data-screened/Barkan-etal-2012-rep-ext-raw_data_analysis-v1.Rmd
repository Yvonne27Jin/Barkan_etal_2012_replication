...
cleaned_df <- ethi_dis_write[5:42] %>% 
  rbind(ethi_dis_nowrite[5:42]) %>%
  rbind(con_worthy[5:42]) %>%
  rbind(con_neutral[5:42]) %>%
  rbind(con_nega[5:42]) 
...
# output cleaned data
write.csv(cleaned_df, "cleaned_data_0719_screened.csv",fileEncoding = "UTF-8")
```
...
```{R Study3 MASC1 output, eval=FALSE, include=TRUE}
...
```{R Study3 set2 output, eval=FALSE, include=TRUE}
...
```{R Study3 MASC Set3 output, eval=FALSE, include=TRUE}
...
```{R Study3 BIDR self ouput, eval=FALSE, include=TRUE}
...
```{R Study3 BIDR impression output, eval=FALSE, include=TRUE}
