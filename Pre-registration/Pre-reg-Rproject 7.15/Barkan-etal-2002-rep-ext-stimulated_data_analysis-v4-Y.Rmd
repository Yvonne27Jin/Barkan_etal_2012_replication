---
title: "Barkan etal (2012) data analysis using randomly generated data - V4"
author: "Yvonne JIN"
date: "7/14/2022"
output:
...
```{r read data}
df <- read_sav("NOCHECK - [RRR] Barkan et al. (2012) Ethical dissonance [Yvonne Yaqi Jin] - V2_June 22, 2022_09.35.sav")
```
...
ethi_dis_write <- df %>% filter(FL_9_DO_RecallManipulation_EthicalDissonancebyWriting_UnethicalB == 1) %>%
  dplyr::select("study_order",
         ...
         age:CountryName) %>% #demographic data and condition marker
...
# output cleaned data
write.csv(cleaned_df, "stimulated_cleaned_data.csv",fileEncoding = "UTF-8")
```
...
```{R ManiCheck output, eval=FALSE, include=TRUE}
...
```{R study1DV1 outout, eval=FALSE, include=TRUE}
...
```{R Study1DV2 output, eval=FALSE, include=TRUE}
...
```{R Study1DV3 output, eval=FALSE, include=TRUE}
...
```{R Study2DV1 plot, eval=FALSE, include=TRUE}
ggsave(
...
```{R Study2DV2 plot, eval=FALSE, include=TRUE}
ggsave(
...
```{R Study2DV3 plot, eval=FALSE, include=TRUE}
ggsave(
...
```{R Study3 MASC1 plot, eval=FALSE, include=TRUE}
ggsave(
...
```{R Study3 set2 plot, eval=FALSE, include=TRUE}
ggsave(
...
```{R Study3 MASC Set3 plot, eval=FALSE, include=TRUE}
ggsave(
...
```{R Study3 BIDR self plot, eval=FALSE, include=TRUE}
ggsave(
...
```{R Study3 BIDR impression plot, eval=FALSE, include=TRUE}
ggsave(
